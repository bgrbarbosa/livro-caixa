package br.com.bgrbarbosa.livro_caixa_api.controller.exception;



import br.com.bgrbarbosa.livro_caixa_api.service.exception.DatabaseException;
...
public class ResourceExceptionHandler {
	...
	public ResponseEntity<StandardError> database(TransientObjectException e, HttpServletRequest request) {
		...
		err.setTimestamp(Instant.now());
		err.setStatus(status.value());
		err.setError("error saving the record, check the values and try again");
		err.setMessage(e.getMessage());
		err.setPath(request.getRequestURI());
		return ResponseEntity.status(status).body(err);
	}
